...
if __name__ == '__main__':
    ...
    print("\nThis is the pose classification script."
          "\nTo train a pose, press and hold any of the a-p keys while making a pose.\n")
            ...
            if HAVE_PYGAME:
                for ev in pygame.event.get():
                    if ev.type == QUIT or (ev.type == KEYDOWN and ev.unicode == 'q'):
                        ...
                        #if K_0 <= ev.key <= K_9:
                        #THE SECOND IN IF KEY DEPENDS ON myo.NUM_POSES
                        if K_a <= ev.key <= K_p:
                            #hnd.recording = ev.key - K_0
                            hnd.recording = ev.key - K_a
                            print("TRAINING POSE: ", hnd.recording)
                        ...
                        if K_a <= ev.key <= K_p: #or K_KP0 <= ev.key <= K_KP9:
                            print("STOPPED TRAINING POSE: {} \n".format(hnd.recording))
                            hnd.recording = -1
                
##                hnd.recording = input("Enter pose to train, 99 to quit: ")
##                if hnd.recording != 99:
##                    print("Training pose: ", hnd.recording)
##                else:
